...
/**
 * Classe utilitaire pour generer le code d'un document a partir de ses
 * informations
 */
public class CodeGenerator {
	...
	public static String generateCodeLetter(String chaine) {
		...
		if (chaine != null) {
			if (items.length >= 2) { // Premiere Partie du Code = Premiere lettre Nom Prenom Auteur
				returnCode = items[0].substring(0, 1).toUpperCase(); // IE : Patrick Senecal = PS-*****-**
				returnCode += items[1].substring(0, 1).toUpperCase();
			} else if ((chaine.length() >= 2) && !Character.isDigit(chaine.charAt(0))) { // Si 1 item Code = 2 premieres
																							// lettres du nom
				returnCode = chaine.substring(0, 2).toUpperCase(); // IE : Corneille = CO-*****-**
			} else
				// Si 1 item commencant par un chiffre : les 2 caracteres à partir de la
				// position 1
